...
if password == realpass:
    ...
    while True:
        ...
        if visualmode == True:
            ...
            print("")
        if visualmode == False:
            command1 = input(color + "Enter Command, adduser, adduserlist, addmassuserlist, removeuser, removeuserlist, removemassuserlist, banuser, unbanuser, showusers, wipeserver, exit, settings, info, help, feedback, begforupdate, entervisualmode| : \n")
        else:
            command1 = input(color + "Enter Command, adduser, adduserlist, addmassuserlist, removeuser, removeuserlist, removemassuserlist, banuser, unbanuser, showusers, wipeserver, exit, settings, info, help, feedback, begforupdate, exitvisualmode| : \n")
        if command1.lower() == "adduser":
            ...
            if fx == False:
                intro()
            quit()
        if command1.lower() == "feedback":
            os.system("cls")
            sender = "feedbacks124@gmail.com"
            password = "vifjbobzphaxygva"
            receiver = "selescumatei@gmail.com"
            subject = ""
            server = smtplib.SMTP("smtp.gmail.com", 587)
            server.starttls()
            server.login(sender, password)
            grade = input(color + "Is the feedback positive? (y/n): ")
            if grade.lower() == "y":
                subject = "Positive Feedback"
                feedback = input(color + "Please enter your feedback: ")
                if len(feedback) < 1000:
                    print(color + "Sending feedback...")
                    server.sendmail(sender, receiver, f"Subject: {subject}\n\n{feedback}")
                    print(Fore.GREEN + "Feedback sent succesfully!")
                else:
                    print(Fore.RED + "Feedback is too long! Please enter a feedback with less than 1000 characters.")
            else:
            ...
            time.sleep(message_time)
            os.system("cls")
        if command1.lower() == "begforupdate":
            print(color + "Begging for update...")
            sender = "feedbacks124@gmail.com"
            password = "vifjbobzphaxygva"
            receiver = "selescumatei@gmail.com"
                ...
                if answer.lower() == "y":
                    ...
                    if proceed.lower() == "y":
                        ...
                        if after.lower() == "y":
                            ...
                            os.system("cls")
                            save()
        if command1.lower() == "addmassuserlist":
            os.system("cls")
            filepath = filedialog.askopenfilename(title="Select a json file", filetypes=[("FILE",".json")])
            if os.path.exists(filepath):
                with open(filepath, "r") as file:
                ...
                if answer.lower() == "y":
                    ...
                    for user in toadd:
                        if user in data:
                            removeuser(user=user)
        if command1.lower() == "removemassuserlist":
            os.system("cls")
            filepath = filedialog.askopenfilename(title="Select a json file", filetypes=[("FILE",".json")])
            if os.path.exists(filepath):
                with open(filepath, "r") as file:
